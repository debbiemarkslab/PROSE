...
class PoET(nn.Module, LogitsAllocateMemoryMixin):
    ...
    @torch.inference_mode()
    def sample_given_human_and_memory(
        self,
        memory: Optional[list[PackedTensorSequences]],
        temperature: float = 1,
        top_k: Optional[int] = None,
        ...
        while True:
            ...
            # <<< CHANGE START >>>
            # Condition the first token *after* the start token to be human_token
            if current_position_int == 0:
                next_token = torch.full_like(next_token, alphabet.human_token)
            # <<< CHANGE END >>>
            ...
            if is_stop_batch_filter.sum() > 0:
                ...
                sampled_scores.append(
                    -criteria.forward(
                        current_logits[is_stop_batch_idxs].transpose(1, 2),
                        current_x[is_stop_batch_idxs, 1:].cuda(), # Assuming .cuda() is intended if device is cuda
                    )
                    .float()
                    .sum(dim=1)
                )
