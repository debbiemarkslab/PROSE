...
from protriever import util, dist_utils

from protriever.index_io import load_or_initialize_index, save_embeddings_and_index
from protriever.model_io import create_checkpoint_directories, load_or_initialize_protriever_model, save_protriever_model, save_protriever_model_no_fabric, load_dataloader
from protriever.options import get_options
from evaluate_poet import run_evaluation
from lightning.fabric import Fabric
from lightning.pytorch.loggers import CSVLogger, TensorBoardLogger, WandbLogger
from lightning.fabric.strategies import DDPStrategy
from protriever.filtering import create_self_match_filter
...
def train(
    ...
    if opt.use_gradient_checkpoint_reader:
        model.reader.gradient_checkpointing_enable(gradient_checkpointing_kwargs={"use_reentrant":False})
        logger.info(f"Checking gradient checkpointing for reader: {model.reader.is_gradient_checkpointing}")

    if opt.use_gradient_checkpoint_retriever:
        model.retriever.gradient_checkpointing_enable(gradient_checkpointing_kwargs={"use_reentrant":False})
        logger.info(f"Checking gradient checkpointing for retriever: {model.retriever.is_gradient_checkpointing}")
    ...
    # Create the filtering function once
    self_match_filter = create_self_match_filter()
    ...
    for step in range(total_steps + 1):
        ...
        if not opt.use_file_passages and index_refresh_scheduler.is_time_to_refresh(step):
            ...
            if step==0 and index.is_index_trained():
                ...
                logger.info("Building initial index from already calculated embeddings.")
        ...
        if opt.final_finetune_steps > 0 and step >= (opt.total_steps - opt.final_finetune_steps):
        ...
        # print(f'train_loss is {train_loss}')
        if retriever_loss is not None and opt.train_retriever:
            iter_stats["loss/reader_loss"] = (reader_loss.item() * opt.accumulation_steps, batch_size)
            iter_stats["loss/retriever_loss"] = (retriever_loss.item() * opt.accumulation_steps, batch_size)
        iter_stats["loss/train_loss"] = (train_loss.item() * opt.accumulation_steps, batch_size)
        ...
        if step% opt.log_freq == 0 and step > 0:
            log = util.log_training_metrics(step, run_stats.average_stats, fabric, scheduler=scheduler, opt=opt)
            logger.info(log)
            run_stats.reset()

        # Saving checkpoints
        if step % opt.save_freq == 0 and step > 0 or step == opt.total_steps or step == total_steps:
            checkpoint_name = (
                "final-pretrain" if step == opt.total_steps
                else "final-finetune" if step == total_steps
                else f"step-{step}"
            )
            ...
            #     fabric
            # )
            save_protriever_model_no_fabric(model, optimizer, scheduler, retr_optimizer, retr_scheduler, step, opt, checkpoint_path, checkpoint_name, fabric)

        # Evaluation: add and step > 0 to avoid eval at step 0
        if step % opt.eval_freq == 0 and step > 0 or step == opt.total_steps or step == total_steps:
            eval_prefix = (
                "[Final Pretrain] " if step == opt.total_steps
                else "[Final Finetune] " if step == total_steps
                else ""
...
if __name__ == "__main__":
    ...
    checkpoint_path, saved_index_path, saved_initial_index_path = create_checkpoint_directories(opt)


    # Initialize Fabric loggers
    tb_logger = TensorBoardLogger(save_dir=os.path.join(opt.checkpoint_dir, "tensorboard"))
    ...
    if opt.wandb_key is not None:
        os.environ["WANDB_API_KEY"] = opt.wandb_key  # Ensure this environment variable is set
        wandb_logger = WandbLogger(project=opt.wandb_project, name=opt.name, config=vars(opt))
    ...
    fabric = Fabric(accelerator="gpu", 
                    ...
                    # devices=4,
                    # strategy="ddp",
                    strategy=DDPStrategy(process_group_backend="gloo", find_unused_parameters=True, static_graph=static_graph)
                    )
    ...
    model, optimizer, scheduler, retr_optimizer, retr_scheduler, opt, step = load_or_initialize_protriever_model(opt, fabric, eval_only=False)
    ...
    if retr_optimizer is not None:
        model, optimizer, retr_optimizer = fabric.setup(model, optimizer, retr_optimizer)
    else:
        model, optimizer = fabric.setup(model, optimizer)

    logger.info(f"fabric has set up model and optimizer")
    model.mark_forward_method('build_index')  # Changed from model.build_index to 'build_index'
